...
**Manifest (section 8).** `.scsh.yml` is skills-only — no `version`, `project`, or `image` headers. Its `skills:` section lists exactly five reviewer directories under `.skills/*-reviewer/` (each with a `SKILL.md` whose `name` matches its directory). Each reviewer entry declares `profile: code-gorgeous-review`, `timeout`, `result: tmp/code-review-<reviewer>-{name}.json`, and an **`invocations:`** block with three routes — codex Terra (`gpt-5.6-terra`, `effort: high`), claude Opus (`claude-opus-4-8`), and cursor Auto (`auto`) — each with `harness` and `model`. `scsh run --profile code-gorgeous-review` expands those to fifteen invocations named `{reviewer}-{route}`. The gorgeous-specific profile name remains distinct from a consumer repository's legacy `code-review` profile. The manifest also includes `internal-self-check-reviewers` with direct run fields, `profile: internal-self-check`, and `autoinstall: false`. No reviewer directory is missing; no reviewer lacks the three invocation routes.
