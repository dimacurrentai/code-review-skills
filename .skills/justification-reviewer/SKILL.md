...
The result is a single JSON object at `$SCSH_RESULT` when it is set (**only** there), else at `tmp/code-review-justification-reviewer.json` — produced by the shipped writer described below, never by hand:
...
```sh
SKILL_DIR='.skills/justification-reviewer'  # wherever this SKILL.md actually lives
